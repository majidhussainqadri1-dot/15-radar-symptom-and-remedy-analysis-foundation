<?php defined( 'ABSPATH' ) || exit; $terms = get_the_terms( $entry_id, SRF_Helpers::TAX ); ?>
<article class="srf-card" data-radar-entry="<?php echo absint( $entry_id ); ?>">
	<div class="srf-card-top"><span class="srf-type"><?php echo esc_html( $terms && ! is_wp_error( $terms ) ? $terms[0]->name : 'Radar Entry' ); ?></span><?php if ( $can_save ) : ?><label class="srf-select"><input type="checkbox" value="<?php echo absint( $entry_id ); ?>" data-srf-select-entry> Select for Study</label><?php endif; ?></div>
	<h3><a href="<?php echo esc_url( get_permalink( $entry_id ) ); ?>"><?php echo esc_html( get_the_title( $entry_id ) ); ?></a></h3>
