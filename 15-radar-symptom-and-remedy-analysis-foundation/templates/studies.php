    ...
    <?php else : ?>
        ...
        <section class="rsr-results"><div class="rsr-section-heading"><h2><?php esc_html_e('My studies', RSR_TEXT_DOMAIN); ?></h2><span><?php echo esc_html((string)count((array)$view['studies']['items'])); ?></span></div><div class="rsr-card-grid" data-rsr-study-list>
            <?php foreach ((array)$view['studies']['items'] as $study) : ?>
                <article class="rsr-card rsr-study-card" data-study='<?php echo esc_attr(wp_json_encode($study)); ?>'><header><h3><?php echo esc_html((string)$study['title']); ?></h3><span class="rsr-status"><?php echo esc_html((string)$study['status']); ?></span></header><p><?php echo esc_html(RSR_Domain::explain_query((array)$study['query'])); ?></p><p class="rsr-meta"><?php echo esc_html((string)$study['updated_at']); ?></p><div class="rsr-card-actions"><button class="rsr-text-button" type="button" data-rsr-study-edit><?php esc_html_e('Edit', RSR_TEXT_DOMAIN); ?></button><button class="rsr-text-button rsr-danger" type="button" data-rsr-study-delete><?php esc_html_e('Delete', RSR_TEXT_DOMAIN); ?></button></div></article>
            <?php endforeach; ?>
        </div></section>
    <?php endif; ?>
</div>
