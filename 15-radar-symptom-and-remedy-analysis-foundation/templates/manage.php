            ...
            <?php if (!empty($view['can_manage_sources'])) : ?>
                <div class="rsr-inline-editor" hidden data-rsr-source-editor>
                    ...
                    <form data-rsr-source-form>
                        ...
                        <div class="rsr-field"><label><?php esc_html_e('License', RSR_TEXT_DOMAIN); ?><select name="license_code"><?php foreach (RSR_Domain::writable_source_licenses() as $license) : ?><option value="<?php echo esc_attr($license); ?>"><?php echo esc_html($license); ?></option><?php endforeach; ?></select></label></div>
                        ...
                        <?php foreach ((array)$view['reports'] as $report) : ?>
                            <tr data-report='<?php echo esc_attr(wp_json_encode($report)); ?>'>
                                ...
                                <td><div class="rsr-action-row">
                                    <?php if (!empty($view['can_review_reports'])) : foreach (['analyst_review', 'editorial_review'] as $state) : ?><button type="button" class="rsr-text-button" data-rsr-report-transition="<?php echo esc_attr($state); ?>"><?php echo esc_html(str_replace('_', ' ', ucfirst($state))); ?></button><?php endforeach; endif; ?>
                                    <?php if (!empty($view['can_approve_reports'])) : ?><button type="button" class="rsr-text-button" data-rsr-report-transition="approved"><?php esc_html_e('Approve', RSR_TEXT_DOMAIN); ?></button><?php endif; ?>
