...
<div class="rsr-page rsr-compare-page" dir="<?php echo is_rtl() ? 'rtl' : 'ltr'; ?>">
    ...
    <section class="rsr-panel">
        <form method="get" action="<?php echo esc_url($view['urls']['compare']); ?>" class="rsr-inline-form" data-rsr-manual-compare>
            <?php for ($i = 0; $i < 3; $i++) : ?>
        ...
        <?php else : ?>
            <section class="rsr-results">
                <div class="rsr-card-grid rsr-remedy-heads">
                    <?php foreach ((array)$view['comparison']['remedies'] as $remedy) : ?><article class="rsr-card"><h2><?php echo esc_html((string)$remedy['title']); ?></h2><?php if (!empty($remedy['url'])) : ?><a class="rsr-text-link" href="<?php echo esc_url((string)$remedy['url']); ?>"><?php esc_html_e('Canonical entry', RSR_TEXT_DOMAIN); ?></a><?php endif; ?><p><?php echo wp_kses_post((string)($remedy['summary'] ?? '')); ?></p></article><?php endforeach; ?>
                </div>
                <section class="rsr-panel"><h2><?php esc_html_e('Common reviewed rubrics', RSR_TEXT_DOMAIN); ?></h2><?php if (empty($view['comparison']['common_rubrics'])) : ?><p><?php esc_html_e('No common reviewed mapping is currently available.', RSR_TEXT_DOMAIN); ?></p><?php else : ?><div class="rsr-table-wrap"><table class="rsr-table"><thead><tr><th><?php esc_html_e('Rubric', RSR_TEXT_DOMAIN); ?></th><th><?php esc_html_e('References', RSR_TEXT_DOMAIN); ?></th></tr></thead><tbody><?php foreach ((array)$view['comparison']['common_rubrics'] as $rubric) : ?><tr><td><?php echo esc_html((string)$rubric['explanation']); ?></td><td><?php foreach ((array)$rubric['remedies'] as $reference) : ?><span class="rsr-reference-chip"><?php echo esc_html((string)$reference['reference']); ?> · <?php echo esc_html((string)$reference['review_date']); ?></span><?php endforeach; ?></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?></section>
                <section class="rsr-panel"><h2><?php esc_html_e('Differing reviewed rubrics', RSR_TEXT_DOMAIN); ?></h2><?php if (empty($view['comparison']['differing_rubrics'])) : ?><p><?php esc_html_e('No differing reviewed mapping is currently available.', RSR_TEXT_DOMAIN); ?></p><?php else : ?><div class="rsr-accordion-list"><?php foreach ((array)$view['comparison']['differing_rubrics'] as $rubric) : ?><details><summary><?php echo esc_html((string)$rubric['explanation']); ?></summary><ul><?php foreach ((array)$rubric['remedies'] as $remedy_id => $reference) : ?><li><strong><?php echo esc_html((string)$remedy_id); ?></strong>: <?php echo esc_html((string)$reference['reference']); ?> · <?php echo esc_html((string)$reference['license']); ?></li><?php endforeach; ?></ul></details><?php endforeach; ?></div><?php endif; ?></section>
                <div class="rsr-notice"><?php echo esc_html((string)$view['comparison']['interpretation']); ?></div>
            </section>
        <?php endif; ?>
    <?php endif; ?>
</div>
